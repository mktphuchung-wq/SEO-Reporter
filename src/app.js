...
app.get("/team", async (req, res) => {
  try {
    ...
    const rows = members.map((m)=>`<tr>${td(`<a href="/team/${m.id}">${escapeHtml(m.name)}</a>`)}${td(escapeHtml(m.email||"—"))}${td(escapeHtml(m.default_property_url||"—"))}${td(n(m.url_count))}${td(m.latest_job_status?renderStatusBadge(m.latest_job_status):"—")}${td(n(m.latest_current_clicks))}${td(n(m.latest_current_impressions))}${td(`<form method="post" action="/team/${m.id}/run-quarterly" style="display:inline"><button class="btn btn-secondary" type="submit">Run quarterly report</button></form>`)}</tr>`);
    ...
    res.type("html").send(teamLayout(req,{title:"Team Members · SEO Reporter",pageTitle:"Team Members",pageDescription:"Manage internal users, their GSC properties, and URL lists.",body}));
  } catch (error) { res.status(500).type("html").send(teamLayout(req,{title:"Team error",pageTitle:"Team Members",body:renderAlert({type:"error",message:safeErrorMessage(error)})})); }
});
...
app.get("/team/:memberId", async (req,res)=>{ const [member, lists, jobs]=await Promise.all([getTeamMember(req.params.memberId),listUrlListsForMember(req.params.memberId),listQuarterlyJobs({memberId:req.params.memberId})]); if(!member) return res.status(404).send("Not found"); const form=`<form class="card" method="post"><div class="grid grid-2"><div class="field"><label>Name</label><input name="name" value="${escapeHtml(member.name)}" required></div><div class="field"><label>Email</label><input name="email" value="${escapeHtml(member.email||"")}"></div></div><div class="field"><label>Default property</label><input name="defaultPropertyUrl" value="${escapeHtml(member.default_property_url||"")}"></div><button class="btn">Update member</button></form>`; const listRows=lists.map(l=>`<tr>${td(escapeHtml(l.name))}${td(escapeHtml(l.property_url))}${td(escapeHtml(l.search_type))}${td(n(l.url_count))}${td(renderStatusBadge(l.status))}</tr>`); const jobRows=jobs.map(j=>`<tr>${td(escapeHtml(j.quarter_label))}${td(renderStatusBadge(j.status))}${td(n(j.total_urls))}${td(`${n(j.completed_batches)}/${n(j.total_batches)}`)}${td(`<a href="/team/quarterly-jobs/${j.id}/status">Status</a> · <a href="/team/quarterly-jobs/${j.id}/results">Results</a>`)}</tr>`); res.type("html").send(teamLayout(req,{title:member.name,pageTitle:member.name,body:`<div class="actions"><a class="btn" href="/team/${member.id}/url-list">Add/Edit URL list</a><form method="post" action="/team/${member.id}/run-quarterly"><button class="btn btn-secondary">Run quarterly report</button></form></div>${form}<h2>URL Lists</h2>${table(["Name","Property","Search type","URLs","Status"],listRows)}<h2>Quarterly Jobs</h2>${table(["Quarter","Status","URLs","Batches","Action"],jobRows)}`})); });
app.post("/team/:memberId", async (req,res)=>{ await updateTeamMember(req.params.memberId, req.body); res.redirect(`/team/${req.params.memberId}`); });
app.get("/team/:memberId/url-list", async (req,res)=>{ const lists=await listUrlListsForMember(req.params.memberId); const active=lists.find(l=>l.status==='active')||lists[0]||{}; const urls=Array.isArray(active.urls_json)?active.urls_json.join("\n"):""; const body=`<form class="card" method="post"><div class="field"><label>List name</label><input name="listName" value="${escapeHtml(active.name||"Default URL List")}"></div><div class="field"><label>Property URL</label><input name="propertyUrl" value="${escapeHtml(active.property_url||"")}" required></div><div class="field"><label>Search type</label><select name="searchType"><option value="web">web</option><option value="image">image</option><option value="video">video</option><option value="news">news</option></select></div><div class="field"><label>URLs</label><textarea name="urlsText" required>${escapeHtml(urls)}</textarea><p class="helper">Mỗi URL một dòng. Nếu danh sách dài, app sẽ tự chia batch 50 URL khi chạy quarterly report.</p></div><button class="btn">Save URL list</button></form>`; res.type("html").send(teamLayout(req,{title:"URL List",pageTitle:"URL List",body})); });
app.post("/team/:memberId/url-list", async (req,res)=>{ try { const parsed=parseAndNormalizeUrlList(req.body.urlsText); if(parsed.invalidRows.length) throw new Error(`${parsed.invalidRows.length} invalid URL(s).`); const lists=await listUrlListsForMember(req.params.memberId); const active=lists.find(l=>l.status==='active'); if(active) await updateUrlList(active.id,{name:req.body.listName,propertyUrl:req.body.propertyUrl,searchType:req.body.searchType,urls:parsed.urls}); else await createUrlList({memberId:req.params.memberId,name:req.body.listName,propertyUrl:req.body.propertyUrl,searchType:req.body.searchType,urls:parsed.urls}); await updateTeamMember(req.params.memberId,{defaultPropertyUrl:req.body.propertyUrl}); res.redirect(`/team/${req.params.memberId}`); } catch(error){ res.status(400).type("html").send(teamLayout(req,{title:"URL List",pageTitle:"URL List",body:renderAlert({type:"error",message:safeErrorMessage(error)})})); }});
